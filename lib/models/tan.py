...
#
#
# # def b_generate_proposals_feat(frames_feat, props):
# #         props_feats = []
# #         props_len = []
# #
...
# #                 s, e = int(s) * 1, int(e) * 1
# #                 clip_len = e - s
# #                 idx = np.linspace(start=0, stop=clip_len - 1, num=16).astype(np.int64)
# #                 try:
...
#
#
# # def _generate_proposals_feat(frames_feat, props):
# #     props_feats = []
# #     props_len = []
# #
# #     for f, p in zip(frames_feat, props):
...
# #                 random_feat = torch.rand(16, 500).cuda()
...
# #             s, e = int(s) * 1, int(e) * 1
# #             clip_len = e - s + 1
# #             if s == 0 and e == 0:  # 如果 s 和 e 都是 0
# #                 # 使用随机初始化创建特征
# #                 feat = torch.randn(16, f.size(1)).cuda() # 假设特征的第二维度与 f 相同
# #             else:
# #                 idx = np.linspace(start=0, stop=clip_len - 1, num=16).astype(np.int64)
# #                 try:
# #                     feat = f[s:e + 1][idx]
...
# #                     feat = torch.randn(16, f.size(1)).cuda()  # 假设特征的第二维度与 f 相同
# #
# #             props_feats.append(feat)
# #             props_len.append(feat.size(0))
# #
# #     props_feats = torch.stack(props_feats, 0)
# #     return props_feats, props_len, None
#
...
#                 random_feat = torch.zeros(16, 512).cuda()
#                 props_feats.append(random_feat)
...
#
#             # 生成索引并提取特征
#             idx = np.linspace(start=s_frame, stop=e_frame, num=16, endpoint=False).astype(np.int64)
...
#
#
# # 示例用法（需要提供 frames_feat 和 props）
# # frames_feat = ...
# # props = ...
# # props_feat, props_len, props_mask = _generate_proposals_feat(frames_feat, props)
#
# # def _generate_proposals_feat(frames_feat, props, batch_size, duration):
# #     props_feats = []
# #     # props_len = []
# #
# #     for f, p, d in zip(frames_feat, props, duration):
...
# #             if s == 0 and e == 0:
# #                 # 如果 s 和 e 都是 0，则随机初始化特征
# #                 random_feat = torch.rand(16, 512).cuda()
# #                 props_feats.append(random_feat)
# #                 props_len.append(16)
# #                 continue
# #
# #             s, e = int(s), int(e)
# #             e = min(e, 255)  # 确保 e 的值不超过 256
# #             s=min(s,255)
# #             clip_len = e - s
# #             idx = np.linspace(start=0, stop=clip_len - 1, num=16).astype(np.int64)
# #             idx = np.minimum(idx, 255)  # 如果 idx 中的值大于256，替换为256
# #
# #             try:
# #                 props_feats.append(f[s:e + 1][idx])
# #             except IndexError:
...
# #     # 假设scores的维度为[4, 8, 1, 64, 64]
# #     # 初始化输出列表，确保维度为 (4, 8, 3, 2) 来存储前三个最大值的时间戳
...
# #             if score.sum() > 1e-3:
# #                 T = score.shape[-1]
# #                 # 将score展平并获取前三个最大值的索引
# #                 flat_score = score.view(-1)
# #                 top3_max_indices = torch.topk(flat_score, 3).indices  # 获取前三个最大值的索引
# #
...
# #                     # 更新输出列表
# #                     out_max_timestamps[batch_idx, text_idx, i] = torch.tensor(max_timestamp)
# #             # 如果分数矩阵全为0，则对应位置保持为 (0, 0) 不变
# #
# #     return out_max_timestamps
#
#
# def get_max_score_timestamps(scores, durations):
#     # 假设scores的维度为[4, 8, 1, 64, 64]
...
# # def get_proposal_results(scores, durations):
# #     out_sorted_times = []
# #     for batch_idx, batch in enumerate(scores):  # 使用 enumerate 获取批次索引
# #         batch_duration = durations[batch_idx]  # 获取当前批次的持续时间
# #         batch_times = []
# #         for score_sent in batch:  # 遍历批次中的每个文本
# #             # 只取每个score_sent的最大值
# #             max_score_index = torch.argmax(score_sent)
# #             T = score_sent.shape[-1]
# #             row, col = np.unravel_index(max_score_index.cpu().numpy(), (T, T))
# #
# #             # 如果最大值索引有效（即满足row <= col）
# #             if row <= col:
# #                 sorted_index = np.array([[row, col + 1]]).astype(float)  # 加1是因为原代码中有这样的操作
# #                 sorted_index = torch.from_numpy(sorted_index).cuda()
# #                 target_size = config.DATASET.NUM_SAMPLE_CLIPS // config.DATASET.TARGET_STRIDE
# #                 time = (sorted_index.float() / target_size * batch_duration).tolist()
# #                 batch_times.append(time)
# #             else:
# #                 # 如果最大值索引无效，可以选择添加一个空列表或其他操作
# #                 batch_times.append([])
# #
# #         out_sorted_times.append(batch_times)
# #
# #     return out_sorted_times
# #
...
#         d_model = 128
#         self.pos_feat = torch.zeros((d_model*3, 8, 64, 64))
...
#         self.fc_comp = nn.Linear(512, config.DATASET.VOCAB_SIZE)
...
#         # visual_input: (b,256, input_size) i.e.(64,256,500)
...
#         # print(map_h.shape)torch.Size([2, 512, 64, 64])
...
#         # print(fused_h.shape)torch.Size([2，8, 512, 64, 64])
...
#         # print(prediction.shape)torch.Size([2, 8, 1, 64, 64])
#         map_mask = map_mask.view(batch_size, 8, 1, map_size, map_size)
#         # print(map_mask.shape)torch.Size([2, 8, 1, 64, 64])上三角为1，下三角全是0。
# ################################################################################计算最佳的提案所对应的融合特征#####################################################################
#         tmp_shape = prediction.shape
#         joint_prob = torch.sigmoid(prediction) * map_mask
#         # print(joint_prob.shape)torch.Size([2, 8, 1, 64, 64])
...
#         # print(targets.shape)torch.Size([4, 8, 1, 64, 64])
#
...
#         # 调整 fused_h 的形状
#         fused_h_reshaped = fused_h.view(batch_size, 8, 512, 64, 64)
...
#         # print(results.shape)
#         # print(mask_squeezed.shape)  # torch.Size([batch_size, 8, 1])
#         # print(mask_squeezed)
#         # target token
...
#
#         vg_hs_t = self.vl_transformer2(vl_src_t, vl_mask_t, vl_pos_t)  # (1+L+N)xBxC
#         vg_hs_t = vg_hs_t[0]
#         # print(vg_hs_t.shape)
# ###################################################################################################
...
#         props_feat, props_len, props_mask = _generate_proposals_feat(visual_output.unsqueeze(1).repeat(1,8,1,1).view(batch_size * 8,256,512), jj,duration_tensors)
#         # print(props_len)16
#
#
#         # print(props_feat)
...
#         # print(words_feat.shape)batch_size * 8,aa,512
#         words_feat1 = words_feat
#         words_feat2 = txt_h_a.view(batch_size * 8,aa,512)
...
#         # words_feat2 = words_feat2.unsqueeze(1) \
#         #     .expand(bsz, num_proposals, -1, -1).contiguous().view(bsz * num_proposals, words_mask1.size(1), -1)
#         props_feat1=props_feat
#         fc= nn.Linear(512,512).cuda()
...
#         # torch.Size([64, 24])
#
#         fc_comp = nn.Linear(512, config.DATASET.VOCAB_SIZE).cuda()
#         # print(words_feat1.shape)torch.Size([96, 13, 512])
#         # print(props_mask)None
#
#         _, h = self.trans(props_feat1, props_mask, words_feat1, words_mask1, decoding=2)
#         # print(h.shape)torch.Size([64, 24, 512])
#
#         # words_logit = fc_comp(h)
#         words_logit=self.fc_comp(h)
# ########################################################wenben rec##############################################################
#         _, h_2 = self.trans(words_feat2, textual_mask1, words_feat, textual_mask1, decoding=2)
#
#         words_logit2 = self.fc_comp(h_2)
#
# #####################################################################################算每个句子的预测时间区域#############################################
...
#         # joint_prob = torch.sigmoid(prediction) * map_mask
#         # # print(joint_prob.shape)torch.Size([4, 8, 1, 64, 64])
#         # # print(duration)
#         # sorted_times = get_proposal_results(joint_prob, duration)
#         # print(sorted_times)
#         # for segs in zip(sorted_times):
#         #     # assert len(segs) == len(dats['timestamps']) or len(segs) == 8
#         #
#         #     for seg in zip(segs):
#         #         seg = nms(seg, thresh=config.TEST.NMS_THRESH, top_k=1).tolist()
#         #         print(seg)
#         #
...
#         # max_recall = max(recalls)
#         # count = 0
#         # for segs, dats in zip(joint_prob, duration):
#         #     # assert len(segs) == len(dats['timestamps']) or len(segs) == 8
#         #     if not (len(segs) == 8):
#         #         count += 1
#         #         print(count)
...
#         #             print(len(segs))
#         #         continue
#         #     for seg, dat in zip(segs, dats['duration'][:len(segs)]):
...
#         fused_h_t = fused_h_t.view(batch_size, 8, 512, 64, 64)
...
#                     feature_z = fused_h_t[batch, sentence, :, :, :].view(1, 512, 64, 64)
#
...
#                     p_values_batch_z.append(torch.zeros(1, 1, 64, 64).cuda())
#
...
#
#                 if sentence_mask[batch, sentence, 0, 0, 0] == 1:
#                     feature_f = fused_h_t[batch, sentence, :, :, :].view(1, 512, 64, 64)
#
#                     # 如果这不是第一个有效的特征且prev_pp存在，则将prev_pp乘以当前的feature
...
#                 else:
#                     # 如果当前句子被遮罩，将零填充张量添加到列表中
#                     p_values_batch_f.append(torch.zeros(1, 1, 64, 64).cuda())
#
...
#         # print(y_t_f.shape)torch.Size([4, 8, 1, 2])
#         # print(y_t_z.shape)torch.Size([4, 8, 1, 2])
#         overlaps_tensor_f = calculate_overlaps(duration_tensor, y_t_f, num_clips, num_sentences, batch_size)
#
#         weights=None
#         return prediction, map_mask, sims, self.logit_scale, jj, weight_3, words_logit,words_logit2, ids_list, weights, words_mask1, overlaps_tensor_z,p_values_tensor_z,overlaps_tensor_f,p_values_tensor_f
    ...
    for f, p, d in zip(frames_feat, props, duration):
        ...
        for s, e in p:
            if s == 0 and e == 0 and d==0:
                # 如果 s 和 e 都是 0，则随机初始化特征
                # random_feat = torch.normal(0,0.1,size=(16,512)).cuda()
                random_feat = torch.zeros(32, 512).cuda()
                props_feats.append(random_feat)
                # props_len.append(batch_size * 8)
                continue
            ...
            # 生成索引并提取特征
            idx = np.linspace(start=s_frame, stop=e_frame, num=32, endpoint=False).astype(np.int64)
            idx = np.minimum(idx, total_frames - 1)  # 确保索引不超出范围
            try:
                props_feats.append(f[idx])
            except IndexError:
...
class TAN(nn.Module):
    ...
    def forward(self, textual_input, textual_mask, sentence_mask, visual_input,duration,weights_list,ids_list):
        ...
        map_size = map_h.size(3)
        # different due to dense
        map_mask_1=np.zeros((batch_size,1,64,64))
        for i in range(batch_size):
            np.fill_diagonal(map_mask_1[i,0],1)
            map_mask_1[i,0] = np.triu(np.ones((64,64)))
...
##############################################################################fffffff#########################################################################################3
        ...
        weights=None
        # return prediction, map_mask, self.logit_scale, jj, weight_3, words_logit, ids_list, weights, words_mask1
        return prediction, map_mask, sims, self.logit_scale, jj, weight_3, words_logit, ids_list, weights, words_mask1, overlaps_tensor_f, p_values_tensor_f
