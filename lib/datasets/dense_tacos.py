...
with open('/home/l/data_2/wmz/3-b/DepNet_ANet_Release/data/tacos/glove.pkl', 'rb') as fp:
    vocabs = pickle.load(fp)

class DenseTacos(data.Dataset):
    ...
    def __init__(self, split):
        super(DenseTacos, self).__init__()
        ...
        self.keep_vocab = dict()
        indexs = 0
        for w, _ in vocabs['counter'].most_common(config.DATASET.VOCAB_SIZE):
...
#####################################################################################################################################3333
    ...
    def get_video_features(self, vid):
        vid_key = vid.split('.')[0]
        hdf5_file = h5py.File(os.path.join(self.data_dir, '{}_features.hdf5'.format(self.vis_input_type)), 'r')
        features = torch.from_numpy(hdf5_file[vid_key][:]).float()
        if config.DATASET.NORMALIZE:
            features = F.normalize(features,dim=1)
        vis_mask = torch.ones((features.shape[0], 1))
        return features, vis_mask