...
def dense_collate_fn(batch):
    ...
    batch_weights_list = [b['weights_list'] for b in batch]
    batch_ids_list = [b['ids_list'] for b in batch]
    # weights_list = sample['weights_list']
    ...
    # batch_weights_list (b, 8, seq, 1)
    padded_batch_weights_list = torch.zeros(len(batch), 8, seq, 1)
    for i, weights in enumerate(batch_weights_list):
        n_s = min(weights.shape[0], 8)
        seq_s = weights.shape[1]
        padded_batch_weights_list[i, :n_s, :seq_s] = weights[:n_s]

    #填充id的掩码
    padded_batch_ids_list = torch.zeros(len(batch), 8, seq, 1)
    for i, ids in enumerate(batch_ids_list):
        n_s = min(ids.shape[0], 8)
        seq_s = ids.shape[1]
        padded_batch_ids_list[i, :n_s, :seq_s] = ids[:n_s]
    ...
    batch_data = {
        ...
        'batch_weights_list':padded_batch_weights_list,
        'batch_ids_list':padded_batch_ids_list
    }
    return batch_data

def collate_fn(batch):
    batch_word_vectors = [b['word_vectors'] for b in batch]
    batch_txt_mask = [b['text_mask'] for b in batch]
    batch_weights_list = [b['weights_list'] for b in batch]
    batch_ids_list = [b['ids_list'] for b in batch]
    batch_map_gt = [b['map_gt'] for b in batch]
    batch_anno_idxs = [b['anno_idx'] for b in batch]
    ...
    batch_data = {
        ...
        'batch_weights_list': nn.utils.rnn.pad_sequence(batch_weights_list, batch_first=True),
        'batch_ids_list': nn.utils.rnn.pad_sequence(batch_ids_list, batch_first=True),
        'batch_map_gt': padded_batch_map_gt,
        'batch_vis_input': nn.utils.rnn.pad_sequence(batch_vis_feats, batch_first=True).float(),
        'batch_duration': batch_duration,
    }
...
from lib.datasets.dense_activitynet import DenseActivityNet
from lib.datasets.dense_charades import DenseCharades
from lib.datasets.dense_tacos import DenseTacos
